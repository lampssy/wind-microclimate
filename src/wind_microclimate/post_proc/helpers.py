import os, subprocess, multiprocessing, glob


def postproc_background(case, it, path_vr, output_vr, pv_input, logfile,
        vr_pictures=False, vr_receptors=False):
    ...
    if (vr_pictures and not os.path.exists(csv_vr)) \
    or (vr_receptors and not os.path.exists(csv_vr_receptors)):
